...
### Rule 5: Fork Customization — Branding Is Operator-Configurable

This fork allows the operator to customize the rendered site identity at
runtime (admin console: SystemName, Logo, Footer; templated into index.html
via `router/index-injector.go`). Do **not** hardcode the operator's chosen
brand into source — keep customization driven by `common.SystemName`,
`common.Logo`, `common.Footer`, and other admin-managed options.

Upstream identifiers (Go module path `github.com/QuantumNous/new-api`,
package names, import paths, CI/CD upstream-tracking references) should be
preserved when syncing from upstream to keep the fork merge-friendly. The
embedded `<title>New API</title>` literal in `web/{default,classic}/index.html`
is intentionally kept untouched so upstream rebases stay clean; the runtime
injector replaces it on the fly.
