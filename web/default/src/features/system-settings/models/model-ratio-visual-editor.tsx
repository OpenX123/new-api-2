...
>(function ModelRatioVisualEditor(
  ...
  const models = useMemo(() => {
    ...
    const emptySnapshot = (name: string): ModelPricingSnapshot => ({
      name,
      price: '',
      ratio: '',
      cacheRatio: '',
    ...
    const rows = Array.from(modelNames)
      .map((name) => {
        const saved = savedByName.get(name)
        const draft = draftByName.get(name)
        const displayed = saved ?? draft ?? emptySnapshot(name)
    ...
    if (filterMode === 'unset') {
      return rows.filter((row) => {
        const effective = row.draft ?? row
        return (
          effective.billingMode !== 'tiered_expr' &&
          !hasPricingValue(effective.price) &&
          !hasPricingValue(effective.ratio)
        )
      })
    }
    return rows
