...
export const channelFormSchema = z
  .object({
    ...
    hide_upstream_error_message: z.string().optional(),
  ...
  if (channel.setting) {
    try {
      ...
      extraSettings = {
        ...
        hide_upstream_error_message: parsed.hide_upstream_error_message || '',
      }
    } catch (error) {
...
function buildSettingJSON(formData: ChannelFormValues): string {
  const settingObj = {
    ...
    hide_upstream_error_message: formData.hide_upstream_error_message || '',
  }
...
export function transformFormDataToCreatePayload(formData: ChannelFormValues): {
  ...
  const channel: Partial<Channel> = {
    ...
    max_concurrency: formData.max_concurrency || null,
