...
func OaiResponsesToChatBufferedStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	if err := scanner.Err(); err != nil {
		if ttftErr := helper.VisionTTFTError(err); ttftErr != nil {
			return nil, ttftErr
		}
...
func OaiResponsesToChatStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	state := relayconvert.NewResponsesToChatStreamState(helper.GetResponseModelName(info), false)
	state.ID = responseId
	...
	scannerErr := helper.StreamScannerHandler(c, resp, info, func(data string, sr *helper.StreamResult) {
		...
		if info.IsModelMapped && streamResp.Response != nil {
			// hide mapped model name: keep the client-facing model in state
			streamResp.Response.Model = ""
		}
